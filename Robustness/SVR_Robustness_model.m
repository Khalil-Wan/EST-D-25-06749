...
%  add noise
[m_noise, n_noise]=size(p_test);

% Add different noise to different features, take Level III as an example.
Data_noise(:,1)  =p_test(:,1)+0.03*randn(m_noise,1);% Temperature
Data_noise(:,122)=p_test(:,122)+0.05*randn(m_noise,1);% CT
Data_noise(:,123)=p_test(:,123)+0.05*randn(m_noise,1);% DT
Data_noise(:,124)=p_test(:,124)+0.05*randn(m_noise,1);% CE
% Frequence
